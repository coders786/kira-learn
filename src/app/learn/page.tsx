...
export default function LearnPage() {
  ...
  useEffect(() => {
    ...
    if (last.role === "ai" && !voice.isSpeaking) {
      voice.speak(last.content, user?.personality || "chill");
    }
  }, [messages.length, autoSpeak]);

  // Always-on voice: auto-send when final transcript detected
  useEffect(() => {
    if (!alwaysOnVoice || !voice.transcript) return;
    // Detect "final" — transcript hasn't changed in 2 seconds
    const timer = setTimeout(() => {
      if (voice.transcript && voice.transcript.split(/\s+/).length >= 2) {
        handleSend(voice.transcript, true);
        voice.clearTranscript();
      }
    }, 2000);
    return () => clearTimeout(timer);
  }, [voice.transcript, alwaysOnVoice]);

  // Toggle always-on
  useEffect(() => {
    voice.setAlwaysOn(alwaysOnVoice);
    if (alwaysOnVoice && !voice.isListening && step === "teaching") {
  ...
  function detectMood(content: string): MoodType {
    const now = Date.now();
    responseTimes.current.push(now - lastMsgTime.current);
    ...
    if (avg < 3000 && /\b(got it|next|yep|cool|makes sense)\b/.test(lower)) return "in-the-zone";
    return "normal";
  }
  ...
  // ===== SEND =====
  const handleSend = useCallback(async (content: string, isVoiceInput = false) => {
    ...
    if (special) {
      addMessage({ id: uid(), role: "user", content: sanitized.clean || content.trim(), timestamp: Date.now(), phase: step as ConversationPhase, isVoice: isVoiceInput });
    ...
    addMessage({ id: uid(), role: "user", content: sanitized.clean, timestamp: Date.now(), phase: step === "teaching" ? "teaching" : "goal-discovery", isVoice: isVoiceInput });
    setIsLoading(true);
  ...
  }, [isLoading, orchestrator, step, user, messages, addMessage, screenShare]);
  ...
  const handlePersonality = useCallback((p: PersonalityType) => {
    ...
    addMessage({ id: uid(), role: "ai", content: `okay. i'll be ${c?.name}. let's do this.\n\none thing — i can see your screen while we learn. you turn it off anytime. i don't save anything. just look, help, forget. want to try?`, timestamp: Date.now(), phase: "screen-permission" });
  }, [user, addMessage]);

  const handleScreenPerm = useCallback(async (yes: boolean) => {
    if (yes) { try { await screenShare.startSharing(); } catch {} }
    setStep("teaching"); setOnboarded(true);
    addMessage({ id: uid(), role: "user", content: yes ? "yes share" : "not now", timestamp: Date.now(), phase: "screen-permission" });
    if (user && orchestrator) {
      setIsLoading(true);
      try {
        const ctx = { phase: "teaching" as ConversationPhase, messages, user, mistakes: getMistakes(), quests: getQuests(), progressNotes: getProgressNotes(), mood: "normal" as MoodType, sessionStart: Date.now(), isScreenSharing: yes };
        const r = await orchestrator.generateResponse(ctx, `${yes ? "yes" : "no"} screen. i'm ready to learn ${user.tool}. where do we begin?`);
  ...
  const handleReset = useCallback(() => {
    ...
    greeted.current = false; setOnboarded(false);
  }, []);
    ...
    <div className="h-screen flex flex-col bg-kira-bg overflow-hidden">
      ...
      <header className="flex-shrink-0 flex items-center justify-between px-3 sm:px-5 h-12 border-b border-kira-border/50 glass">
        ...
        <div className="flex items-center gap-1.5">
          {/* Auto-speak */}
          <button onClick={() => setAutoSpeak(!autoSpeak)} className={`p-1.5 rounded-lg transition-all text-[11px] ${autoSpeak ? "bg-kira-accent/20 text-kira-accent" : "text-kira-textMuted/30 hover:text-kira-textMuted"}`} title={autoSpeak ? "AI voice ON" : "AI voice OFF"}>
          ...
          {/* Always-on voice */}
          <button onClick={() => setAlwaysOnVoice(!alwaysOnVoice)} className={`p-1.5 rounded-lg transition-all text-[11px] ${alwaysOnVoice ? "bg-kira-green/20 text-kira-green" : "text-kira-textMuted/30 hover:text-kira-textMuted"}`} title={alwaysOnVoice ? "Always-on voice" : "Push-to-talk"}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
              <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" />
              <path d="M19 10v2a7 7 0 0 1-14 0v-2" />
              <line x1="12" y1="19" x2="12" y2="23" />
              {alwaysOnVoice && <circle cx="12" cy="12" r="11" strokeDasharray="3 3" />}
            </svg>
          </button>
          {/* Screen */}
          <button onClick={() => screenShare.isSharing ? screenShare.stopSharing() : screenShare.startSharing()} className={`p-1.5 rounded-lg transition-all ${screenShare.isSharing ? "bg-kira-accent/20 text-kira-accent" : "text-kira-textMuted/40 hover:text-kira-textMuted"}`} title={screenShare.isSharing ? "Stop sharing" : "Share screen"}>
          ...
          {/* Mic */}
          <button onClick={handleVoiceToggle} className={`p-1.5 rounded-lg transition-all ${voice.isListening ? "bg-kira-accent/20 text-kira-accent pulse-ring relative" : voice.isSpeaking ? "bg-kira-green/10 text-kira-green" : "text-kira-textMuted/40 hover:text-kira-textMuted"}`} title="Voice">
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        {showSidebar && <Sidebar user={user} isScreenSharing={screenShare.isSharing} onShowMistakes={() => { setShowMistakes(true); setShowSidebar(false); }} onShowQuest={() => { setShowQuest(true); setShowSidebar(false); }} onReset={handleReset} onClose={() => setShowSidebar(false)} />}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {step === "screen-permission" && (
            <div className="border-t border-kira-border/50 bg-kira-surface/50 p-5 animate-fade-up">
              <div className="max-w-2xl mx-auto flex gap-3">
                <button onClick={() => handleScreenPerm(true)} className="flex-1 py-3 bg-kira-accent hover:bg-kira-accentLight text-white rounded-xl transition-all btn-glow font-medium">yes, share my screen</button>
                <button onClick={() => handleScreenPerm(false)} className="py-3 px-5 bg-kira-surface border border-kira-border text-kira-textMuted rounded-xl hover:text-kira-text transition-colors">skip</button>
              </div>
            </div>
          )}
          ...
          <div className="flex-shrink-0 border-t border-kira-border/50 p-3 glass">
            <form onSubmit={e => { e.preventDefault(); if (inputText.trim()) handleSend(inputText.trim()); }} className="flex items-center gap-2.5 max-w-2xl mx-auto">
              {voice.isSpeaking && <button type="button" onClick={voice.stopSpeaking} className="p-2 rounded-lg bg-kira-red/10 text-kira-red flex-shrink-0" title="Stop"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="2" /></svg></button>}
              <input type="text" value={voice.isListening ? (voice.transcript || "listening...") : inputText} onChange={e => { if (!voice.isListening) setInputText(e.target.value); }} placeholder={voice.isListening ? "speak..." : step === "greeting" ? "what are you trying to figure out?" : "type..."} disabled={isLoading} maxLength={2000}
                className={`flex-1 bg-kira-surface border border-kira-border rounded-xl px-3.5 py-2.5 text-sm text-kira-text placeholder:text-kira-textMuted/40 focus:outline-none focus:border-kira-accent/40 transition-all ${voice.isListening ? "border-kira-accent/40" : ""} ${isLoading ? "opacity-50" : ""}`} />
        ...
        {/* Screen preview */}
        {screenShare.isSharing && <ScreenSharePanel videoRef={screenShare.videoRef} onStop={screenShare.stopSharing} onCapture={screenShare.captureFrame} />}
      </div>
